...
import {
  Search, Hash, X, Loader2, Package, Globe, ShieldCheck, ChevronRight,
} from 'lucide-react';
...
export function ProductExplorer({ countries }: ProductExplorerProps) {
  ...
  /* ── fetch HTS product description when code is selected ── */
  useEffect(() => {
    ...
    return Array.from(allDates).sort().map(date => {
      ...
      for (const d of countryData) {
        ...
        if (rate) {
          ...
          for (const a of AUTHORITIES) row[`${d.country.code}_${a.key}`] = rate[a.key];
      ...
      <CardContent className="p-5 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-4">
          ...
          <div className="space-y-1.5">
                  ...
                  {htsResults.length > 0 ? (
                    htsResults.map((code, index) => (
                      <button key={code} id={`${htsListboxId}-${code}`}
                        ...
                        type="button" role="option" aria-selected={highlightedHtsIndex === index}
                        className={cn(
            ...
            {/* HTS description breadcrumb */}
            {productInfo && productInfo.descriptions.length > 0 && (
              <div className="flex flex-wrap items-center gap-x-1 gap-y-0.5 text-[10px] text-gray-500 leading-relaxed mt-1">
          ...
          <div className="space-y-1.5">
            ...
            <CountryAutocomplete
              countries={countries} value={addingCountry} onChange={setAddingCountry}
        ...
        <div className="flex flex-wrap items-center gap-2">
          {selectedCountries.map((c, i) => (
            <Badge key={c.code} variant="outline"
              className="text-[10px] font-medium pl-1.5 pr-1 py-0.5 gap-1 border-gray-200">
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: COMPARISON_COLORS[i % COMPARISON_COLORS.length] }} />
              {c.name}
              <button type="button" onClick={() => removeCountry(c.code)}
                className="text-gray-400 hover:text-red-500 ml-0.5" title={`Remove ${c.name}`}>
        ...
        {!isLoading && hasData && (
          <>
              ...
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 mt-0.5">
                    Total tariff rate by country, step-wise at each revision boundary
                  </p>
                </div>
              </div>
                          ...
                          <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-xs min-w-[280px]">
                            ...
                            {payload
                              ...
                              .map(p => {
                                ...
                                return (
                                  <div key={cc} className="mb-2.5 last:mb-0">
                                        ...
                                        {AUTHORITIES.filter(a => {
                                          ...
                                          <div key={a.key} className="flex justify-between text-gray-500" title={a.description}>
                                            <div className="flex items-center gap-1">
                                              ...
                                              {a.ch99Prefix && <span className="text-[9px] text-gray-300 font-mono">Ch.99</span>}
                                            </div>
                                            <span className="font-mono">{formatRateShort(d[`${cc}_${a.key}`] as number)}</span>
                                          </div>
                                        ))}
                                        {(() => {
                                        ...
                                        {(() => {
                                          ...
                                          return (usmca || (ms != null && ms > 0 && ms < 1)) ? (
                                            <div className="flex gap-1.5 mt-0.5">
                                              {usmca ? <span className="text-[9px] text-emerald-600 bg-emerald-50 rounded px-1">USMCA</span> : null}
                                              {ms != null && ms > 0 && ms < 1 && (
                                                <span className="text-[9px] text-gray-400 bg-gray-50 rounded px-1">Metal: {(ms * 100).toFixed(0)}%</span>
                                              )}
                                            </div>
                                          ) : null;
                                        })()}
            ...
            <div>
              <h4 className="text-xs font-semibold text-gray-900 mb-1">Full Duty Stack — Latest Revision</h4>
              <p className="text-[10px] text-gray-400 mb-3">
                Complete rate decomposition: MFN base, all Ch. 99 authorities, and trade program eligibility
              </p>
                        ...
                        return (
                          <tr key={d.country.code} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                            <td className="px-3 py-2.5">
                              ...
                              {d.rates.length === 0 && <span className="text-[10px] text-amber-500 ml-5">No data</span>}
                            </td>
                              ...
                              <div className="flex flex-wrap gap-1 justify-center">
                                ...
                                {r && r.metal_share === 1 && (
                                  <span className="text-[9px] bg-orange-50 text-orange-700 border border-orange-200 rounded px-1.5 py-0.5 whitespace-nowrap"
                                    title="Pure metal — 232 takes full precedence.">
                                    Pure metal
                                  </span>
                                )}
                              </div>
